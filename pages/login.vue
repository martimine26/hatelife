<template lang="html">
  <div class="container">
    <h2 style="text-align:center">Добро пожаловать на Activae Vitae! <br><br> Войдите пожалуйста в ваш аккаунт</h2>
    <input type="email" name="name" placeholder="Email" id="login">
    <input type="password" name="password" placeholder="Пароль" id="password">
    <nuxt-link to="/" class="in">Зарегистрироваться</nuxt-link>
    <button type="button" name="button" @click="sen()">Войти</button>
  </div>
</template>
...
export default {
  methods:{
    sen(){
      ...
      const requestURL = 'https://activae-vitae.herokuapp.com/login'
      ...
      function sendRequest(method, url, body = null) {
          return new Promise((resolve, reject) => {
            ...
            xhr.onload = () => {
              ...
              if(stas.code == 400 || stas.code == 403){
                alert("Имя или пароль введены неправильно.Повторите попытку")
              }else{
                console.log(stas)
                console.log(stas.user_id);
                if(localStorage.getItem('token') != null ||localStorage.getItem('id') != null){
                  localStorage.clear()
                  localStorage.setItem('token', stas.token);
                  localStorage.setItem('id', stas.user_id);
                  localStorage.setItem('name', stas.full_name);
                  localStorage.setItem('role', stas.role);
                }else{
                  localStorage.setItem('token', stas.token);
                  localStorage.setItem('id', stas.user_id);
                  localStorage.setItem('name', stas.full_name);
